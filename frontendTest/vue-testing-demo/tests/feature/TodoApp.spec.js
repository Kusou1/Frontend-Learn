...
describe("删除任务", () => {
  test("点击任务项中的删除按钮，任务应该被删除", async () => {
    ...
    await wrapper.setData({
      todos: [{ id: 1, text: "eat", done: false }],
    });
...
describe("切换单个任务完成状态", () => {
  test("选中任务完成状态按钮，任务的样式变成已完成状态", async () => {
    await wrapper.setData({
      todos: [{ id: 1, text: "play", done: false }],
    });
  ...
  test("取消选中任务完成状态按钮，任务的样式变成未完成状态", async () => {
    await wrapper.setData({
      todos: [{ id: 1, text: "play", done: true }],
    });
...
describe("数据筛选", () => {
  ...
  test("点击 all 链接，应该展示所有任务，并且 all 链接应该高亮", async () => {
    ...
    router.push("/");
    await Vue.nextTick();
    expect(wrapper.findAll('[data-testid="todo-item"]').length).toBe(
      filterTodos.all().length
    );
    expect(wrapper.find('[data-testid="link-all"]').classes()).toContain(
      linkActiveClass
    );
  });
  ...
  test("点击 active 链接，应该展示所有任务，并且 active 链接应该高亮", async () => {
    ...
    expect(wrapper.find('[data-testid="link-active"]').classes()).toContain(
      linkActiveClass
    );
  });
  ...
  test("点击 completed 链接，应该展示所有任务，并且 completed 链接应该高亮", async () => {
    ...
    expect(wrapper.find('[data-testid="link-completed"]').classes()).toContain(
      linkActiveClass
    );
  });
});
